...
words1.forEach((item ,index)=>{
   ...
   errorWords=(words1.length-cnt);
  
});
return `${cnt}  words correct out of ${words1.length} words and the total number of error words are ${errorWords}.` ;
}   

const endPlay=()=>{
    let date=new Date();
    endTime=date.getTime();
    let toatalTime=((endTime-stratTime)/1000);
    let totalStr=typeWords.value;
    // console.log(typeWords.value);
    let wordCount=wordCounter(totalStr);
    let speed=Math.round((wordCount/toatalTime)*60);
    let finalMsg=`You Typed total at  speed ${speed}  words per minutes`;
    finalMsg+=compareWords(msg.innerText,totalStr.trimStart());
